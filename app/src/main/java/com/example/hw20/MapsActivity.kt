...
import com.example.hw20.model.Reminder2
...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    private fun setLongClick(map: GoogleMap) {
        map.setOnMapLongClickListener { latlng ->
            ...
            if (key != null) {
                val reminder = Reminder2(key, latlng.latitude, latlng.longitude)
                reference.child(key).setValue(reminder)
            }
            createGeoFence(latlng, key!!, geofencingClient)

            val intent = Intent(applicationContext, AddFragment::class.java)
            intent.putExtra("lat", latlng.longitude)
